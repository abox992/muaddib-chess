...
int generateMoves(const Board &board, Move *moveList) {
  ...
  const int kingPos =
      tz_count(board.getBB(Piece::KINGS + static_cast<int>(color)));
...
inline int generatePawnMoves(const Board &board, Move *moveList,
  ...
  while (bitboard) { // bitloop
    ...
    if ((initialAttackMask & enpassantSquareMask) &&
        board.enpassantPos() > 0) {
      ...
      uint64_t rookCompressedBlockers =
          extract_bits(blockers, rookMasks[kingPos]);

      blockers = adjustedPieces & bishopMasks[kingPos];
      uint64_t bishopCompressedBlockers =
          extract_bits(blockers, bishopMasks[kingPos]);

      uint64_t kingAttackers =
          (pawnAttackMasks[color][kingPos] & opPawns) |
    ...
    if ((pinMask & currentSquareMask) != 0) { // might be pinned
      for (int i = 0; i < 8; i++) {           // loop through all directions
        ...
        if (std::popcount(axis & board.getAll<color>()) ==
            1) { // more than 1 piece means were not actually pinned
          pLegalMoves &= axis;
    ...
    while (
      ...
      if (board.enpassantPos() > 0 &&
          index == board.enpassantPos()) { // enpassant
...
inline int generateKnightMoves(const Board &board, Move *moveList,
  ...
  while (bitboard) { // bitloop
    ...
    uint64_t pLegalMoves = knightMasks[currentSquare] &
                           (board.getEmpty() | board.getAll<enemyColor>());
    ...
    if ((pinMask & currentSquareMask) != 0) { // might be pinned
      for (int i = 0; i < 8; i++) { // loop through all directions/axis'
        ...
        if (std::popcount(axis & board.getAll<color>()) ==
            1) { // more than 1 piece would mean were not actually pinned
          pLegalMoves &= axis;
          break; // can only be pinned on a single axis, stop checking if we
                 // find one
        }
...
inline int generateBishopMoves(const Board &board, Move *moveList,
  ...
  while (bitboard) { // bitloop
    ...
    uint64_t compressedBlockers =
        extract_bits(blockers, bishopMasks[currentSquare]);

    uint64_t pLegalMoves = bishopLegalMoves[currentSquare][compressedBlockers];
    pLegalMoves &=
        ~board.getAll<color>(); // blockers above assumes we can take any piece,
                                // remove captures of our own pieces
...
inline int generateRookMoves(const Board &board, Move *moveList,
  ...
  while (bitboard) { // bitloop
    ...
    uint64_t compressedBlockers =
        extract_bits(blockers, rookMasks[currentSquare]);
...
inline int generateQueenMoves(const Board &board, Move *moveList,
  ...
  while (bitboard) { // bitloop
    ...
    uint64_t pLegalMovesHV =
        rookLegalMoves[currentSquare][rookCompressedBlockers];
    uint64_t pLegalMovesDiag =
        bishopLegalMoves[currentSquare][bishopCompressedBlockers];
...
inline int generateKingMoves(const Board &board, Move *moveList,
  ...
  uint64_t pLegalMoves =
      kingMasks[kingPos] & (board.getEmpty() | board.getAll<enemyColor>());

  // castles
  if (~checkMask == 0) {          // can only castle if not in check
    if (board.canCastle(color)) { // king side
      if ((board.getOccupied() & castleMasks[color]) ==
          0) { // no pieces in the way
        if (attacksOnSquare<color, false>(board, kingPos - 1) ==
            0) { // cannot pass through check
          pLegalMoves |= originalRookSquares[color]; // we encode as taking rook
                                                     // on king side
        }
      }
    }
    ...
    if (board.canCastle(color + 2)) { // queen side
      if ((board.getOccupied() & castleMasks[color + 2]) ==
          0) { // no pieces in the way
        if (attacksOnSquare<color, false>(board, kingPos + 1) ==
            0) { // cannot pass through check
          pLegalMoves |=
              originalRookSquares[color +
                                  2]; // we encode as taking rook on queen side
        }
      }
    }
  }
  ...
  while (pLegalMoves) {
    ...
    if (maskForPos(index) &
        board.getBB(Piece::ROOKS + static_cast<int>(color))) { // taking our own rook -> castle move
      curMove = Move::make<CASTLE>(kingPos, index);
      // std::cout << "made castle move" << std::endl;

      int pos =
          0; // tz_count(maskForPos(index) & board.getBB(Piece::ROOKS + static_cast<int>(color)));
