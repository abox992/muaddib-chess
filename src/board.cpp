...
void Board::makeMove(const Move& move) {
    ...
    if (move.moveType() == MoveType::CASTLE) {
        ...
        this->curState->pieces[ROOKS + static_cast<int>(color)] |= Bitboard::castleRookSquares[pos]; // add new rook pos
        this->curState->pieces[ROOKS + static_cast<int>(color)] &= ~Bitboard::originalRookSquares[pos]; // remove old rook pos
        ...
        this->curState->hash ^= Zobrist::randomTable[tz_count(Bitboard::castleRookSquares[pos])][ROOKS + static_cast<int>(color)]; // add new rook pos
        // this->curState->hash ^= Zobrist::randomTable[from][KINGS + static_cast<int>(color)]; // remove old king pos
        this->curState->hash ^= Zobrist::randomTable[this->kingPos(color)][KINGS + static_cast<int>(color)]; // add new king pos
    }
    ...
    if (piece == ROOKS) { // rook move, can no longer castle on that side
        if ((Bitboard::originalRookSquares[color] & this->curState->pieces[colorPiece]) == 0) { // if king side rook not on original square
            this->curState->canCastle[color] = false;
        }
