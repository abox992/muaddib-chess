...
constexpr inline bool isOk(const int square) { return square < 64 && square >= 0; }
...
uint64_t attacksOnSquare(const Board& board, int square) {
    ...
    uint64_t kingAttackers =
      (Bitboard::pawnAttackMasks[color][square] & opPawns) | (Bitboard::knightMasks[square] & opKnights)
      | (Bitboard::bishopLegalMoves[square][bishopCompressedBlockers] & opBQ)
      | (Bitboard::rookLegalMoves[square][rookCompressedBlockers] & opRQ) | (Bitboard::kingMasks[square] & opKing);
...
uint64_t generateCheckMask(const Board& board) {
    int kingPos =
      board
        .kingPos<color>();  // tz_count(board.curState->pieces[Piece::KINGS + color]);//board.getPiecePos(10 + color);
